...
\end{equation}
where $P_N$ is the set of permutations of the $N$-set $\left\{1,\dots,N\right\}$
...
N^2)$ if implemented naively, and with $\mathcal{O}(2^N N)$ if the sets $S$ in Equation
...
\end{equation}
where $\text{adj}_{+}$ is the permanental adjugate, wherein the $i,j$-minors of $A$ are computed
with permanents instead of determinants.  We can also compute the derivative of a permanent with
respect to a matrix element $a_{ij}$ by using a special case of Equation (\ref{eq:jacobis}),
\begin{equation}
